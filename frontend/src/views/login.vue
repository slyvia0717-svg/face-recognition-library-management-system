<template>
  <main class="auth-page">
    ...
    <section class="auth-card">
      ...
      <p class="eyebrow">重逢 · 续一程书香</p><h1>管理员登录</h1><p class="auth-intro">登录管理账号，进入图书馆工作台。</p>
      ...
      <FaceLogin :nickname="nickname" :admin-only="true" :disabled="busy" @busy="faceBusy = $event" />
      <p class="auth-bottom">借阅图书？<router-link to="/login2">前往借阅者登录</router-link></p>
    </section>
  </main>
</template>
<script setup>
...
async function submitForm() {
  ...
  try {
    const user = await login(nickname.value, password.value, true);
    password.value = '';
